...
active_bar <- bar_df %>%
  ...
  mutate(condition = factor(condition, levels = c("UpINS", "DownINS", "UpSCP1", "DownSCP1")),
        ...
        label = case_when(enhancer_type == "active" & condition == "UpINS" ~ "1,329\n11.40%",
                          enhancer_type == "active" & condition == "DownINS" ~ "967\n8.30%",
                          enhancer_type == "active" & condition == "UpSCP1" ~ "1,132\n9.71%",
                          enhancer_type == "active" & condition == "DownSCP1" ~ "1,075\n9.22%",
                          TRUE ~ ""),
        label_pos = 20,
